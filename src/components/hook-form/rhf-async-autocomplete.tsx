...
interface RHFAsyncAutoCompleteProps {
  ...
  fetchUrl: string;
  debounceTime?: number;
  getOptionLabel: (option: any) => string; // Function to extract the label from the option
}
...
const RHFAsyncAutoComplete: React.FC<RHFAsyncAutoCompleteProps> = ({
  ...
  const fetchOptions = debounce(async (searchTerm: string, nextPage: number) => {
    try {
      setLoading(true);
      const response = await axiosInstance1.get(fetchUrl, { params: { search: searchTerm, page: nextPage } });
      const newOptions = response.data?.data;
      setOptions((prevOptions: any[]) => {
        const filteredOptions = newOptions.filter((newOption:any) => !prevOptions.some(prevOption => prevOption.id === newOption.id));
